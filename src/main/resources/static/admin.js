...
function addUserToTable(user, row) {
    ...
    trUsers.insertAdjacentHTML('beforeend',
        `<td>${user.id}</td>
            ...
            <td></td>
            <td>
                <button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#modalUserEdit">
                    Edit
                </button>
            </td>
            <td>
...
function fillModal(event, modal, actionPrefix) {
    ...
    if (actionPrefix == 'edit') {
        modal.querySelector(`#${actionPrefix}Password`).value = '';
    }
...
createForm.addEventListener('submit', event => {
    ...
    const button = document.getElementById('user-table-tab');
